/**
This is a class that creates a directed, weighted graph data structure of elements of generic type E where every node is connected to all other nodes.
@author Nathan Mankovich
*/
public class DGraph <E extends Comparable<E>> {
	...
	*/
	...
	/**
	This method does a sort of breadth first search to find the optimal path throught the graph based on edge weights.
	@param index is the starting index in the data array
	*/
	public void optimalTrav (int index) {
		...
		int prevEdgeVal = -1;
		ofsIndex = this.oft(markedO, index, ofsIndex, prevEdgeVal);
	}
	
	/**
	This method traverses the graph in a permutation of a weighted breadth first using recursion.
	@param markedO a boolean array that marks visited nodes in the traversal as true
	@param index is the current index of the visited nodes in the graph
	@param ofsIndex is a queue of indexes that stores visited nodes
	@return ofsIndex queue of the indexes of the elements in traversal order.
	*/
	private Queue<Integer> oft(boolean[] markedO,int index, Queue<Integer> ofsIndex, int pEdgVal) {
		if (ofsIndex.GetSize() <= size) {
			...
			if (loc != -1) {
				...
				pEdgVal = -1;
				index = loc;
				oft(markedO, index, ofsIndex, pEdgVal);
			}
			else {
				System.out.print("");
			}
	...
	/**
	This method uses the ofsIndex to return the element in the graph at the next index in the breadth first queue.
	@return data[ofsIndex.Dequeue()] e the element at the next index in the ofsIndex queue
	*/
	public E oNextNode() {
