...
public class Graph <E extends Comparable<E>> {
	...
	*/
	...
	public void optimalTrav (int index) {
		...
		Queue<Integer> optQueue = new Queue<Integer>();
		Queue<Integer> pQueue = new Queue<Integer>();
		for (int i = 0; i < size; i++) {
			markedO[i] = false;
		}
		
		markedO[index] = true;
		optQueue.Enqueue(index);
		int prevEdg = -1;
		int cInd = index;
		
		for (int j = 0; j < size; j++) {
			for (int i = 0; i < size; i++) {
				if (edges[index][i] > prevEdg && !markedO[i]) {
					cInd = i;
					prevEdg = edges[index][i];			
				}
			}
			
			if (cInd != -1) {
				markedO[cInd] = true;
				optQueue.Enqueue(cInd);
				pQueue.Enqueue(cInd);
			}
			
			index = optQueue.Dequeue();
		}
		
		while (!pQueue.CheckIfEmpty()) {
